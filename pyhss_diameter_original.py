...
class Diameter:
    ...
    def Answer_16777265_303(self, packet_vars, avps):
        """SWx MAR - Multimedia Authentication Request Handler"""
        self.logTool.log(service='HSS', level='debug', message="[diameter.py] [Answer_16777265_303] [MAR] SWx MAR received", redisClient=self.redisMessaging)
        ...
        avp += self.generate_avp(260, 40, "0000010a4000000c000028af000001024000000c01000031")  # SWx App ID

        try:
            imsi = self.get_avp_data(avps, 1)[0]
            imsi = binascii.unhexlify(imsi).decode('utf-8')
            imsi = imsi.split('@')[0]
            self.logTool.log(service='HSS', level='debug', message=f"[Answer_16777265_303] IMSI: {imsi}", redisClient=self.redisMessaging)
            ...
            vectors = self.database.Get_Vectors_AuC(auc_id, "eap_aka", plmn=plmn)
            self.logTool.log(service='HSS', level='debug', message=f"[Answer_16777265_303] Got vectors: {vectors}", redisClient=self.redisMessaging)
            ...
            # SIP-Auth-Data-Item AVP (612)
            sip_auth_data = ''
            sip_auth_data += self.generate_vendor_avp(608, "c0", 10415, str(binascii.hexlify(b'EAP-AKA'), 'ascii'))  # SIP-Authentication-Scheme
            sip_auth_data += self.generate_vendor_avp(609, "c0", 10415, rand + autn)  # SIP-Authenticate (RAND+AUTN)
            sip_auth_data += self.generate_vendor_avp(610, "c0", 10415, xres)         # SIP-Authorization (XRES)
            sip_auth_data += self.generate_vendor_avp(611, "c0", 10415, ck)           # Confidentiality-Key
            sip_auth_data += self.generate_vendor_avp(612, "c0", 10415, ik)           # Integrity-Key (reusing 612 for IK here - actually AVP 614)

            # Correct AVP codes for CK/IK
            sip_auth_item = ''
            sip_auth_item += self.generate_vendor_avp(608, "c0", 10415, str(binascii.hexlify(b'EAP-AKA'), 'ascii'))
            sip_auth_item += self.generate_vendor_avp(609, "c0", 10415, rand + autn)
            sip_auth_item += self.generate_vendor_avp(610, "c0", 10415, xres)
            sip_auth_item += self.generate_vendor_avp(611, "c0", 10415, ck)
            sip_auth_item += self.generate_vendor_avp(614, "c0", 10415, ik)

            avp += self.generate_vendor_avp(612, "c0", 10415, sip_auth_item)  # SIP-Auth-Data-Item
            avp += self.generate_vendor_avp(607, "c0", 10415, self.int_to_hex(1, 4))  # SIP-Number-Auth-Items
            avp += self.generate_avp(1, 40, str(binascii.hexlify(str.encode(imsi)), 'ascii'))  # User-Name
            ...
            self.logTool.log(service='HSS', level='debug', message="[Answer_16777265_303] MAA generated successfully", redisClient=self.redisMessaging)
            return response

        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[Answer_16777265_303] Error: {e}", redisClient=self.redisMessaging)
            avp_experimental_result = ''
    ...
    def Answer_16777265_301(self, packet_vars, avps):
        """SWx SAR - Server Assignment Request Handler"""
        self.logTool.log(service='HSS', level='debug', message="[diameter.py] [Answer_16777265_301] [SAR] SWx SAR received", redisClient=self.redisMessaging)
        ...
        try:
            ...
            imsi = binascii.unhexlify(imsi).decode('utf-8')
            imsi = imsi.split('@')[0]
            self.logTool.log(service='HSS', level='debug', message=f"[Answer_16777265_301] IMSI: {imsi}", redisClient=self.redisMessaging)

            subscriber_details = self.database.Get_Subscriber(imsi=imsi)
            self.logTool.log(service='HSS', level='debug', message=f"[Answer_16777265_301] Got subscriber: {imsi}", redisClient=self.redisMessaging)
            ...
            self.logTool.log(service='HSS', level='debug', message="[Answer_16777265_301] SAA generated successfully", redisClient=self.redisMessaging)
            return response

        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[Answer_16777265_301] Error: {e}", redisClient=self.redisMessaging)
            avp_experimental_result = ''
            avp_experimental_result += self.generate_vendor_avp(266, 40, 10415, '')
            avp_experimental_result += self.generate_avp(298, 40, self.int_to_hex(5001, 4))
            avp += self.generate_avp(297, 40, avp_experimental_result)
