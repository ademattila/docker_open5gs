...
class Diameter:
    ...
    def __init__(self, logTool, originHost: str="hss01", originRealm: str="ims.mnc000.mcc001.3gppnetwork.org", productName: str="PyHSS", mcc: str="001", mnc: str="000", redisMessaging=None):
        ...
        self.diameterResponseList = [
                {"commandCode": 303, "applicationId": 16777265, "responseMethod": self.Answer_16777265_303, "failureResultCode": 4100 ,"requestAcronym": "MAR", "responseAcronym": "MAA", "requestName": "SWx Multimedia Authentication Request", "responseName": "SWx Multimedia Authentication Answer"},
                {"commandCode": 301, "applicationId": 16777265, "responseMethod": self.Answer_16777265_301, "failureResultCode": 4100 ,"requestAcronym": "SAR", "responseAcronym": "SAA", "requestName": "SWx Server Assignment Request", "responseName": "SWx Server Assignment Answer"},
...
#3GPP SWx Multimedia Authentication Answer (MAA) - RFC 5778 / TS 29.273
    def Answer_16777265_303(self, packet_vars, avps):
        ...
        # Get User-Name AVP (NAI format: 0001010000000001@nai.epc.mnc000.mcc001.3gppnetwork.org)
        try:
            username = self.get_avp_data(avps, 1)[0]
            username = binascii.unhexlify(username).decode('utf-8')
            imsi = username.split('@')[0]
            if imsi.startswith('0') and len(imsi) == 16:
                imsi = imsi[1:]
            self.logTool.log(service='HSS', level='debug', message=f"[SWx] MAR for IMSI: {imsi}", redisClient=self.redisMessaging)
        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[SWx] Failed to parse User-Name AVP: {e}", redisClient=self.redisMessaging)
            return None
        ...
        avp += self.generate_avp(264, 40, self.OriginHost)                                                   #Origin-Host
        avp += self.generate_avp(296, 40, self.OriginRealm)                                                  #Origin-Realm
        avp += self.generate_avp(260, 40, "0000010a4000000c000028af" + "000001024000000c" + format(int(16777265),"x").zfill(8))  #Vendor-Specific-Application-ID SWx

        # Get subscriber from DB
        try:
            subscriber_details = self.database.Get_Subscriber(imsi=imsi)
        except Exception as e:
            self.logTool.log(service='HSS', level='debug', message=f"[SWx] Subscriber {imsi} not found: {e}", redisClient=self.redisMessaging)
            avp_experimental_result = self.generate_vendor_avp(266, 40, 10415, '')
            avp_experimental_result += self.generate_avp(298, 40, self.int_to_hex(5001, 4))   #DIAMETER_ERROR_USER_UNKNOWN
            avp += self.generate_avp(297, 40, avp_experimental_result)
            avp += self.generate_avp(1, 40, str(binascii.hexlify(username.encode()),'ascii'))  #User-Name
        ...
        self.logTool.log(service='HSS', level='debug', message=f"[SWx] Found subscriber: {imsi}", redisClient=self.redisMessaging)

        # Generate EAP-AKA vector
        try:
            mcc = imsi[0:3]
            mnc = imsi[3:5]
            ...
            # AVP 608: SIP-Authentication-Scheme = "EAP-AKA"
            # AVP 609: SIP-Authenticate = RAND + AUTN
            # AVP 610: SIP-Authorization = XRES
            # AVP 611: Confidentiality-Key = CK (from mac+ak for EAP-AKA)
            # AVP 612: Integrity-Key = IK

            sip_auth_data = ''
            
            self.logTool.log(service='HSS', level='debug', message="SWx vector_dict=" + str(vector_dict))
            self.logTool.log(service='HSS', level='debug', message="SWx rand=" + str(vector_dict.get('rand')))
            self.logTool.log(service='HSS', level='debug', message="SWx autn=" + str(vector_dict.get('autn')))
            self.logTool.log(service='HSS', level='debug', message="SWx xres=" + str(vector_dict.get('xres')))
            self.logTool.log(service='HSS', level='debug', message="SWx ck=" + str(vector_dict.get('ck')))
            self.logTool.log(service='HSS', level='debug', message="SWx ik=" + str(vector_dict.get('ik')))
            
            sip_auth_data += self.generate_vendor_avp(608, "c0", 10415, str(binascii.hexlify(b'EAP-AKA'), 'ascii'))
            sip_auth_data += self.generate_vendor_avp(609, "c0", 10415, vector_dict['rand'] + vector_dict['autn'])
            sip_auth_data += self.generate_vendor_avp(610, "c0", 10415, vector_dict['xres'])
            
            if 'ck' in vector_dict and vector_dict['ck']:
                self.logTool.log(service='HSS', level='debug', message="Adding CK AVP 625=" + str(vector_dict['ck']))
                sip_auth_data += self.generate_vendor_avp(625, "c0", 10415, vector_dict['ck'])
            else:
                self.logTool.log(service='HSS', level='debug', message="CK missing or empty")
            
            if 'ik' in vector_dict and vector_dict['ik']:
                self.logTool.log(service='HSS', level='debug', message="Adding IK AVP 626=" + str(vector_dict['ik']))
                sip_auth_data += self.generate_vendor_avp(626, "c0", 10415, vector_dict['ik'])
            else:
                self.logTool.log(service='HSS', level='debug', message="IK missing or empty")
            
            avp += self.generate_vendor_avp(612, "c0", 10415, sip_auth_data)
            avp += self.generate_vendor_avp(607, "c0", 10415, self.int_to_hex(1, 4))
            avp += self.generate_avp(1, 40, str(binascii.hexlify(username.encode()), 'ascii'))
            avp += self.generate_avp(268, 40, self.int_to_hex(2001, 4))

            response = self.generate_diameter_packet("01", "40", 303, 16777265, packet_vars['hop-by-hop-identifier'], packet_vars['end-to-end-identifier'], avp)
            self.logTool.log(service='HSS', level='debug', message="[SWx] Successfully Generated MAA", redisClient=self.redisMessaging)
            return response

        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[SWx] Error generating EAP-AKA vector: {traceback.format_exc()}", redisClient=self.redisMessaging)
            avp += self.generate_avp(268, 40, self.int_to_hex(5012, 4))   #DIAMETER_UNABLE_TO_COMPLY
            response = self.generate_diameter_packet("01", "40", 303, 16777265, packet_vars['hop-by-hop-identifier'], packet_vars['end-to-end-identifier'], avp)
            return response


    def Answer_16777265_301(self, packet_vars, avps):
        self.logTool.log(service='HSS', level='debug', message="[SWx] Got SAR request", redisClient=self.redisMessaging)
        try:
            username = self.get_avp_data(avps, 1)[0]
            username = binascii.unhexlify(username).decode('utf-8')
            imsi = username.split('@')[0]
            if imsi.startswith('0') and len(imsi) == 16:
                imsi = imsi[1:]
            self.logTool.log(service='HSS', level='debug', message=f"[SWx] SAR for IMSI: {imsi}", redisClient=self.redisMessaging)
        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[SWx] SAR failed to parse User-Name AVP: {e}", redisClient=self.redisMessaging)
            return None
        avp = ''
        session_id = self.get_avp_data(avps, 263)[0]
        ...
        avp += self.generate_avp(260, 40, "0000010a4000000c000028af" + "000001024000000c" + format(int(16777265),"x").zfill(8))
        try:
            subscriber_details = self.database.Get_Subscriber(imsi=imsi)
            self.logTool.log(service='HSS', level='debug', message=f"[SWx] SAR subscriber found: {imsi}", redisClient=self.redisMessaging)
        except Exception as e:
            self.logTool.log(service='HSS', level='error', message=f"[SWx] SAR subscriber not found: {e}", redisClient=self.redisMessaging)
            avp_experimental_result = self.generate_vendor_avp(266, 40, 10415, '')
            avp_experimental_result += self.generate_avp(298, 40, self.int_to_hex(5001, 4))
            avp += self.generate_avp(297, 40, avp_experimental_result)
            avp += self.generate_avp(1, 40, str(binascii.hexlify(username.encode()), 'ascii'))
        ...
        avp += self.generate_avp(1, 40, str(binascii.hexlify(username.encode()), 'ascii'))
        avp += self.generate_avp(268, 40, self.int_to_hex(2001, 4))
        self.logTool.log(service='HSS', level='debug', message="[SWx] Successfully Generated SAA", redisClient=self.redisMessaging)
        response = self.generate_diameter_packet("01", "40", 301, 16777265, packet_vars['hop-by-hop-identifier'], packet_vars['end-to-end-identifier'], avp)
        return response
